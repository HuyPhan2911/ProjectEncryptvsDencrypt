# =====================================================
# DES CORE IMPLEMENTATION (FIPS-46-3)
# Academic demo – NOT for real security
# =====================================================
...
class DES:
    def __init__(self, key: bytes):
        ...
        self.key = ''.join(format(b, '08b') for b in key)
        self.round_keys = self._generate_keys()

    def _generate_keys(self):
        key = permute(self.key, PC1)
    ...
    def encrypt_block(self, block: bytes) -> bytes:
        bits = permute(''.join(format(b, '08b') for b in block), IP)
        left, right = bits[:32], bits[32:]

        for i in range(16):
            left, right = right, xor(left, self._feistel(right, self.round_keys[i]))

        final_bits = permute(right + left, FP)
        return int(final_bits, 2).to_bytes(8, 'big')
    ...
    def decrypt(self, data: bytes) -> bytes:
        out = b''.join(self.decrypt_block(data[i:i+8]) for i in range(0, len(data), 8))
        return unpad(out)
